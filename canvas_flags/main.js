...
const flagSelect = document.querySelector("#flagSelect");
const canvas = document.querySelector("#myCanvas");
/* Get the first Element tho show when loading the page */
if (flagSelect && flags.length > 0) {
    const selectedFlag = new Flag(flags[0].countryName, flags[0].countryColors, flags[0].aspectRatio, flags[0].alignment, canvas);
...
flagSelect.addEventListener("change", (e) => {
    var _a;
    const selected = (_a = e.target) === null || _a === void 0 ? void 0 : _a.value;
    if (!selected)
        return;
    const country = flags.find((flag) => flag.countryName === selected);
    if (!country)
        return;
    const selectedFlag = new Flag(country.countryName, country.countryColors, country.aspectRatio, country.alignment, canvas);
    selectedFlag.drawFlag();
});
